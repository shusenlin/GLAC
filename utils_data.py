...
def make_uci_mpu_train_set(ds, num_labeled, num_unlabeled, num_test, seed):
    ...
    labels = Y.argmax(dim=1) # convert one-hot to integer
    ...
    for label in labeled_set:
        ...
        unlabeled_num = torch.ceil(num_unlabeled*class_prior[int(label)]).int()
        labeled_num = torch.ceil(num_labeled*labeled_prior[int(label)]).int()
        test_num = torch.ceil(num_test*class_prior[int(label)]).int()
    ...
    y_unlabeled_train[:] = unlabeled_set[0]
    Y_unlabeled_train = F.one_hot(y_unlabeled_train, len(labeled_set)+1).float() # conver tinteger  to one-hot
